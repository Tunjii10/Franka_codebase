    ...
    def callback(self, data):
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                bounding_box.x_center = float((((xyxy[0]+xyxy[2])/2)-320)*(x/640))
                bounding_box.y_center = float((((xyxy[1]+xyxy[3])/2)-240)*(y/480))
                bounding_box.xmax = int(xyxy[2])
                bounding_box.ymax = int(xyxy[3])
                # int(x1 + abs(x1-x2)/2)
                # bounding_box.xcenter = int(xyxy[0]+ abs(int(xyxy[0])-int(xyxy[2]))/2)
                global bd_box_y, bd_box_x
                bd_box_x = bounding_box.x_center
                bd_box_y = bounding_box.y_center
